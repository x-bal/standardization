...
@section('title', 'Manage Database | Standardization')
@push('css')
...
@push('scripts')
    ...
    <script>
        let url = '';
        let method = '';
        let accountID = "{{ openssl_decrypt(Request::segment(3), 'AES-128-CTR', 'Standardization', 0, '1234567891011121') }}";
        let datableUrl = "{{ route('manage.database.index', ':id') }}";
        ...
        function create(){
            ...
            url = "{{ route('manage.database.store') }}";
            method = "POST";
        }
        function destroy(id){
            let deleteUrl = "{{ route('manage.database.destroy', ':id') }}";
